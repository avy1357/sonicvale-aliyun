<template>
    <div class="page-wrap">
        ...
        <!-- 精校：语音风格指令编辑 -->
        <el-dialog v-model="instructionVisible" title="语音风格指令（精校）" width="500px">
            <p style="color: #909399; font-size: 13px; margin-bottom: 12px;">
                为该行台词设置阿里云 CosyVoice 的语音风格指令，例如："用温柔甜美的语气说"、"愤怒地大喊"、"轻声低语"。
            </p>
            <el-input
                v-model="instructionForm.text"
                type="textarea"
                :rows="4"
                placeholder="请输入语音风格指令，留空则使用角色默认指令"
            />
            <template #footer>
                <el-button @click="instructionVisible = false">取消</el-button>
                <el-button @click="instructionForm.text = ''">清空</el-button>
...
// 判断当前项目是否使用阿里云 TTS
const isAliyunTTS = computed(() => {
    const pid = project.value?.ttsProviderId ?? project.value?.tts_provider_id
    if (!pid) return false
    const provider = ttsProviders.value.find(p => p.id === pid)
    return provider?.provider_type === 'aliyun'
})

// ── 精校（语音风格指令） ──
const instructionVisible = ref(false)
const instructionForm = ref({ lineId: null, text: '' })

function openInstructionEditor(row) {
...
async function saveInstruction() {
    const { lineId, text, _row } = instructionForm.value
    try {
        const res = await lineAPI.updateLine(lineId, {
            chapter_id: _row.chapter_id,
            instruction: text || null,
        })
        if (res?.code === 200) {
            _row.instruction = text || null
            ElMessage.success('语音风格指令已保存')
...
onMounted(async () => {
    ...
    try {
        const ttsRes = await providerAPI.fetchTTSProviders()
        ttsProviders.value = ttsRes || []
    } catch { /* ignore */ }
...
// ── 简易播放（试听列用） ──
const playingLineId = ref(null)
let currentAudio = null

function togglePlay(row) {
    if (playingLineId.value === row.id && currentAudio) {
        currentAudio.pause()
        currentAudio = null
...
function downloadAudio(row) {
    ...
    const a = document.createElement('a')
    a.href = src
    a.download = row.audio_path?.split(/[/\\]/).pop() || 'audio.mp3'
    a.click()
}
            ...
            h('div', {
                ...
                ? [
                    h(ElButton, {
                        circle: true,
                        size: 'large',
                        type: playingLineId.value === rowData.id ? 'danger' : 'primary',
                        icon: playingLineId.value === rowData.id ? VideoPause : VideoPlay,
                        onClick: () => togglePlay(rowData),
                    }),
                    h(ElButton, {
                        circle: true,
                        size: 'large',
        ...
        cellRenderer: ({ rowData }) =>
            isAliyunTTS.value
                ? h('div', { style: 'display:flex;justify-content:center;' }, [
                    h(ElButton, {
                        size: 'small',
                        type: rowData.instruction ? 'success' : 'default',
                        plain: true,
                        onClick: () => openInstructionEditor(rowData),
                    }, () => '精校')
                ])
                : null,
    },
