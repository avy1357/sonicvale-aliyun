    ...
    #         prompt = get_context2lines_prompt(role_names, content,emotion_names,strength_names)
        ...
        #   获取llm_provider
            ...
            try:
                ...
                # 如果 LLM 返回 dict，两种情况：
                # 1. 包装对象如 {"lines": [...]}  → 提取内部列表
                # 2. 单条台词如 {"role_name": "...", "text_content": "..."} → 包装为列表
                if isinstance(parsed_data, dict):
                    logging.info("LLM返回dict，keys: %s", list(parsed_data.keys()))
                    extracted = False
                    for key, value in parsed_data.items():
                        if isinstance(value, list):
                            logging.info("从 dict['%s'] 提取到列表，长度: %d", key, len(value))
                            parsed_data = value
                            extracted = True
                            break
                    if not extracted:
                        # dict 本身可能是一条台词，检查是否含必要字段
                        logging.info("检查dict是否为单条台词，keys: %s, text_content in dict: %s, role_name in dict: %s",
                                   list(parsed_data.keys()),
                                   'text_content' in parsed_data,
                                   'role_name' in parsed_data)
                        if 'text_content' in parsed_data or 'role_name' in parsed_data:
                            logging.info("dict为单条台词，包装为列表")
