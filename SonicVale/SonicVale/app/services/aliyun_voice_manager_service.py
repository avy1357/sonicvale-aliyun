...
class AliyunVoiceManagerService:
    """
    阿里云音色管理服务层
    负责音色的全生命周期管理：查询、创建、更新、删除、批量同步

    与 AliyunVoiceCloneService 的区别：
    - AliyunVoiceCloneService 侧重于声音复刻的创建流程
    - AliyunVoiceManagerService 侧重于音色的统一管理（含批量同步、全量查询等）
    """

    def __init__(self, tts_provider_service: TTSProviderService):
        self.tts_provider_service = tts_provider_service

    def _get_client(self, tts_provider_id: int) -> AliyunVoiceManagerClient:
        """
        根据 TTS 提供商 ID 获取阿里云音色管理客户端

        :param tts_provider_id: TTS 提供商 ID
        :return: AliyunVoiceManagerClient 实例
        """
        ...
        if tts_provider.provider_type != "aliyun":
            raise ValueError("仅支持阿里云 TTS 提供商（provider_type='aliyun'）")

        api_key = getattr(tts_provider, "api_key", None)
        if not api_key:
            raise ValueError("阿里云 TTS 提供商的 API Key 未配置")

        return AliyunVoiceManagerClient(api_key=api_key)
