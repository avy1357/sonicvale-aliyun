...
class LineService:
    ...
    def generate_audio(self, reference_path: str, tts_provider_id, content, emo_text: str, emo_vector: list[float], save_path=None, voice_name: str = None, instruction: str = None):
        ...
        provider_type = getattr(tts_provider, "provider_type", "index_tts") or "index_tts"

        if provider_type == "index_tts":
            return self._generate_with_index_tts(tts_provider, reference_path, content, emo_text, emo_vector, save_path)
        elif provider_type == "volcano":
            return self._generate_with_volcano(tts_provider, content, save_path)
        elif provider_type == "aliyun":
            return self._generate_with_aliyun(tts_provider, content, save_path, voice_name=voice_name, instruction=instruction)
        else:
            raise Exception(f"不支持的TTS提供商类型: {provider_type}")
    
    def _generate_with_index_tts(self, tts_provider, reference_path: str, content: str, emo_text: str, emo_vector: list[float], save_path):
        """使用 IndexTTS 引擎生成音频"""
    ...
    def _generate_with_volcano(self, tts_provider, content: str, save_path):
        x_api_key = getattr(tts_provider, "x_api_key", None)
        api_key = getattr(tts_provider, "api_key", None)
        voice_type = getattr(tts_provider, "voice_type", None)
        resource_id = getattr(tts_provider, "resource_id", None)
        ...
        if x_api_key:
            ...
            app_id = getattr(tts_provider, "voice_clone_appid", None)
            client = VolcanoTTSClient(
                app_key=app_id,
                access_key=api_key,
                resource_id=resource_id,
            ...
            raise Exception("火山引擎 TTS 配置无效，x_api_key 或 api_key 未配置")

        audio_bytes = client.synthesize(content, voice_type)
    ...
    def _generate_with_aliyun(self, tts_provider, content: str, save_path, voice_name: str = None, instruction: str = None):
        """使用阿里云 CosyVoice (DashScope SDK) 生成音频

        鉴权：直接使用 DashScope API Key（与声音复刻共用同一个 Key）
        音色：优先使用台词关联的 voice_name（克隆 voice_id），否则使用 provider 默认 voice_type
        模型：优先使用 api_base_url 字段存储的模型名，否则默认 cosyvoice-v3-plus
        指令：支持自然语言语音风格指令（如"用温柔的语气说"），通过 instruction 参数传递
        """
        api_key = tts_provider.api_key
        if not api_key:
            raise Exception("阿里云 TTS 的 API Key 未配置")

        voice = voice_name or getattr(tts_provider, "voice_type", None)
        if not voice:
            raise Exception("阿里云 TTS 未配置音色，请先在音色管理中绑定音色")

        # api_base_url 复用为模型名配置（阿里云不需要 base URL）
        model = getattr(tts_provider, "api_base_url", "") or "cosyvoice-v3-plus"
        # 兼容旧配置：如果 api_base_url 看起来像 URL 则使用默认模型
        ...
        audio_bytes = client.synthesize(content, voice, instruction=instruction)
